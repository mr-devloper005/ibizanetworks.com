      ...
      <main className="bg-[#07072d] text-white">
        <section className="mx-auto grid min-h-[calc(100vh-12rem)] max-w-[var(--editable-container)] items-center gap-10 px-4 py-12 sm:px-6 lg:grid-cols-[0.9fr_1fr] lg:px-8">
          ...
          <div>
            ...
            <h2 className="mt-5 max-w-xl text-5xl font-black leading-[1.02] tracking-[-0.05em] sm:text-6xl">{pagesContent.auth.signup.title}</h2>
            <p className="mt-6 max-w-lg text-sm leading-8 text-white/68">{pagesContent.auth.signup.description}</p>
            <div className="mt-8 grid max-w-xl gap-3 sm:grid-cols-3">
              {['Free profile', 'Service area', 'Business photos'].map((item) => <span key={item} className="rounded-lg border border-white/15 bg-white/10 px-4 py-3 text-sm font-black">{item}</span>)}
            </div>
          </div>
        </section>
      </main>
